...
def data_process(path, dataset):
    ...
    if dataset in ['Movie']:
        ...
        count = 0
        for i in range(len(seq_dict)):
            if len(seq_dict[i]) > 150:
                count += 1
        print(count)

        # 固定序列长度
        for user in range(len(seq_dict)):
            if len(seq_dict[user]) < 150:  # 填充
                gap = 150-len(seq_dict[user])
                for i in range(gap):  # 直接填充
                    seq_dict[user].append((seq_dict[user][0][0],seq_dict[user][0][1]))
            elif len(seq_dict[user]) == 150:
                seq_dict[user] = seq_dict[user]
            else:
                seq_dict[user] = seq_dict[user][-150:]  # 保留后8个
...
def slid_window(train_data, user_num, item_num, sw_width=5, n_out=5, in_start=0):
    ...
    for u in range(user_num):
        ...
        for in_start in range(50-sw_width):
...
def slid_windowgames(train_data, user_num, item_num, sw_width=5, n_out=5, in_start=0):
    # 该函数实现窗口大小为5、滑动步长为1的滑动窗口截取序列数据
    data = train_data
    adj = torch.zeros((item_num, item_num))

    for u in range(user_num):
        for in_start in range(15):
            in_end = in_start + sw_width  # 14
            out_end = in_end + n_out  # 19
            # 保证截取样本完整，最大元素索引不超过原序列索引，则截取数据；否则丢弃该样本
            if out_end < len(data[u])+n_out+2:  # 18+2=20
...
def indicators_5(rankedList, testList, k):
    Hits_i = 0
    Len_R = 0
    Len_T = len(testList)
    MRR_i = 0
...
def comp_mrr(rankedList, testList, k):
    score = 0.0
