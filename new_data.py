...
def generate_graph(data):
    # groupby进行数据的分组以及分组后地组内运算！ 得到的结果是一个Groupby对象，还没有进行任何的运算。
    ...
    user = data['user_id'].values  # python内置的values()函数返回一个字典中所有的值。
...
def generate_user(user, data, graph, item_max_length, user_max_length, train_path, test_path, k_hop=3, val_path=None):
    data_user = data[data['user_id'] == user].sort_values('time')  # 可以将数据集依照某个字段中的数据进行排序，该函数即可根据指定列数据也可根据指定行的数据排序。
    ...
    if len(u_seq) < 3:
        ...
        for j, t  in enumerate(u_time[0:-1]):
            ...
            # 在j >= item_max_length的子图中，选择了在user的邻居中item_max_length个时间最近的邻居来构建子图。
...
if __name__ == '__main__':
    # 生成全部图，子图（k个邻居和k_hop）,划分训练集和测试集（用户是啥，目标是啥，user和last_item在fin_graph（通过子图）中的索引是啥）
